...
public class EndPlusCommand {

    private static final String GITHUB = "https://github.com/juliuspleunes4/EndPlus";
    ...
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
            dispatcher.register(
                CommandManager.literal("endplus")
                    .then(CommandManager.literal("help")
                        .executes(EndPlusCommand::executeHelp))
                    .then(CommandManager.literal("config")
                        .requires(source -> source.hasPermissionLevel(2))
                        .executes(EndPlusCommand::executeConfig))
            )
        );
    }
    ...
    private static int executeHelp(CommandContext<ServerCommandSource> ctx) {
        ...
        send(source, commandRow("/endplus help  ", "Show this menu"));
        send(source, commandRow("/endplus config", "View active config values  [OP]"));
        send(source, divider());
    ...
    private static int executeConfig(CommandContext<ServerCommandSource> ctx) {
        ServerCommandSource source = ctx.getSource();
        EndPlusConfig cfg = EndPlus.CONFIG;

        send(source, divider());
        send(source, Text.literal(" End+ Config").setStyle(Style.EMPTY.withColor(Formatting.LIGHT_PURPLE).withBold(true)));
        send(source, divider());
