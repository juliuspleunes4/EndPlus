...
public class EndPlusConfig {
    ...
    public static EndPlusConfig load(Path configDir) {
        Path configFile = configDir.resolve("endplus.json");
        if (Files.exists(configFile)) {
            try (Reader reader = Files.newBufferedReader(configFile)) {
                return GSON.fromJson(reader, EndPlusConfig.class);
            } catch (IOException e) {
                return new EndPlusConfig();
            }
        }
        EndPlusConfig defaults = new EndPlusConfig();
        save(defaults, configFile);
        return defaults;
    }

    private static void save(EndPlusConfig config, Path path) {
        try {
